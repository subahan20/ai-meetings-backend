...
const researchAttendee = async (meeting) => {
  const response = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are a research assistant that provides concise professional profiles of people based on their name and email."
      },
      {
        role: "user",
...
const researchCompany = async (meeting) => {
  ...
  const response = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are a business research assistant. Provide a concise company overview including what they do, products/services, and recent notable news."
      },
      {
        role: "user",
...
const analyzeGoals = async (meeting) => {
  const response = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are a meeting strategist. Based on meeting context, suggest clear meeting goals and desired outcomes."
      },
      {
        role: "user",
...
const generateStructuredReport = async (combinedData) => {
  const response = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are an expert executive assistant. Generate a structured meeting prep report."
      },
      {
        role: "user",
...
const generateAudio = async (text) => {
  if (!openai) {
    console.log("Skipping audio generation (OPENAI_API_KEY not found)");
    return null;
  }
